...
\objective{Learn how to use orthogonal transformations to perform QR decomposition.}
\label{lab:givens}

In a previous lab, we discussed how to form the QR decomposition of a matrix using a series of householder reflections. The general approach suggested was to apply a series of orthogonal transformations to a given matrix in order to transform it into a matrix of upper triangular form. This same approach can be applied using rotations instead of reflections.

The matrix 

\centerline{$\begin{pmatrix}\ cos \theta & - \sin \theta \\ \sin \theta & \cos \theta \end{pmatrix}$}

rotates a vector counterclockwise by $\theta$.
We can rotate a given vector $x = \begin{pmatrix} a \\ b \end{pmatrix}$ into the span of $e_1$ by choosing the correct $\theta$. This zeros out the entry containing $b$. 

We could do this by finding the angle between the vector x
%$\begin{pmatrix} a \\ b \end{pmatrix}$ 
and the axis containing $b$ and then rotating the vector by the negative of that angle. However, we want to avoid computations involving $\sin$ and $\arcsin$ at each iteration.  To do so we can solve for $\sin \theta $ and $\cos \theta$ using the pythagorean theorem instead of explicitly computing $\theta$. Such an approach gives us the result that 

\centerline{$\cos \theta = \frac{a}{\sqrt{a^2 + b^2}}$ and $\sin \theta = \frac{- b}{\sqrt{a^2 + b^2}}$.}

The transformed image of the vector x is then $\begin{pmatrix} \sqrt{a^2 + b^2} \\ 0 \end{pmatrix}$.
...
\end{pmatrix}
\end{equation*}
In actual computation, we \emph{do not} actually form such a large matrix at each step. Instead, we apply the transformation to the two rows we want to change and then leave the rest of the matrix unchanged.
...
For example, consider some matrix $A$.
...
\For{each column}
    \For{each row below the main diagonal}
        \If{leading element is not zero}
            ...
            \State Get a slice of the current row of $Q$ and the row above it and apply $G$ to it. (Strictly speaking, you do not need to operate over these entire rows, but the slicing needed to avoid the extra computation is a little more involved, so we will not include that here.)
        \EndIf
    \EndFor
\EndFor
...
\item For each column:
  ...
  \item For each row below the main diagonal (starting at the bottom of the column):
    ...
    \item If the leading entry of this row is not zero (i.e. if its absolute value is within a given tolerance):
      ...
      \item Get a slice of the current row of $Q$ and the row above it and apply $G$ to it. (Strictly speaking, you do not need to operate over these entire rows, but the slicing needed to avoid the extra computation is a little more involved, so we will not include that here.)
...
Because Givens rotations only operate on two specific rows at a time, they provide multiple ways to iterate over the array we are processing. Here are three examples showcasing different iterating processes:

\begin{enumerate}
\item Starting at the bottom of each column and continuing up until you reach the main diagonal, zero out each entry by applying a Givens rotation to each row and the row immediately above it.

\item Starting within each column, zero out the first entry of each row below the main diagonal by rotating it with the row corresponding to the main diagonal.

\item Starting at the bottom left corner of the matrix, zero out each entry from left to right along each diagonal.

\end{enumerate}

This flexibility is what makes using Givens rotations ideal for some problems. When working with sparse matrices, Givens rotations allow for the solution of linear systems through a series of orthonormal operations that operate only on small portions of the array. The fact that Givens rotations operate only on small portions of the array also lends well to parallel solutions to systems of equations (or least squares problems).
...
Write a modified version of your solution to Problem \ref{prob:Givens} called \li{givens2} that computes the QR decomposition of an upper Hessenberg matrix.
...
For approximately what $m$ is your implementation as fast as the general QR decomposition built into \li{scipy.linalg} for computing the QR decomposition of an upper Hessenberg matrix?
\end{problem}

\begin{problem}
\label{prob:givens_hessenberg_modified}
You may have noticed that matrix multiplication by $Q$ is generally a $\mathcal{O} \left( n^3 \right)$ algorithm even though the application of these individual Givens rotations is a $\mathcal{O} \left( n^2 \right)$ algorithm.

Now write a modified version of your solution to Problem \ref{prob:givens_hessenberg} called \li{givens2_mod} which returns 
...
A similar approach would be to store Householder reflectors in the columns they zero out. In either case, we can represent the QR decomposition of an array using only the memory that was originally used to store the array itself.
This is similar to the approach  for computing the LU decomposition entirely in place. These representations of $Q$ and $R$ can be used in various ways to perform matrix multiplication by $Q$, $Q^T$, and $R$ as needed.

\section*{Least Squares}

Let's now talk about line fitting.
...
For Least Squares problems as well as in many other contexts, loading data is often a necessary step before proceeding with further analysis. We will briefly review another data format in Python and the commands used to load the data.

A \li{.npz} file is a compressed binary file that contains an archive of NumPy data structures. A given file may therefore contain several arrays, each array associated with a unique string that identifies it.
When you load a \li{.npz} file in Python, a dictionary-like object is returned and you can access the data by providing the appropriate key. Note that when you load a \li{.npz} file, you must also be sure to close it when
