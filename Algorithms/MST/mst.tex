...
As we add edges to the tree we will change which nodes are the ``root" nodes. This tracks which nodes are currently connected.
...
First, we initialize our spanning tree to be an empty list. Then, to track the root nodes of each tree, we initialize a dictionary where each node points to itself.
...
\begin{lstlisting}
edges = [(C, D, 1), (C, E, 1), (D, E, 2), (A, B, 3), (B, F, 4), (E, F, 4), 
         (B, C, 5), (C, F, 5), (A, F, 6)]
\end{lstlisting}
...
Then we change the root of \li{E} so that it is \li{C}, so the dictionary is
